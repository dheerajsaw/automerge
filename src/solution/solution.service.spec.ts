...
import { change, equals, free, getChanges, init, load } from '@automerge/automerge';
...
describe('SolutionService', () => {
    ...
    describe('create', () => {
        it('should successfully insert a solution', async () => {
            expect(
                await service.create("randomOrgId")
            )
        });
    });
    ...
    describe('update', () => {
        it('should call the update method', async () => {
            let solution = await service.create("randomOrgId")
            let binSolution = new Uint8Array(Buffer.from(solution.encodedSolution, "base64"))
            let loadedSolution = load(binSolution)
            let updatedSolution = change(loadedSolution,
                (solution: any) => {
            ...
            let loadedLatestSolution = load(new Uint8Array(Buffer.from(latestSolution, "base64")))
            expect(equals(loadedLatestSolution, updatedSolution)).toBe(true)
            let updatedSolution1 = change(updatedSolution,
                (solution: any) => {
                    if (!solution.items) solution.items = [];
                    solution.items.push({ abc: "deef", done: false });
                    solution.items.push({ abc: "hello", done: false });
            ...
            let loadedLatestSolution1 = load(new Uint8Array(Buffer.from(latestSolution1, "base64")))
